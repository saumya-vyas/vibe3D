...
import { useAppStore, useObjectStore } from '@/store/useStore'



export function ObjectHighlighter({ onObjectSelected, excludeObjects = [] }) {
  ...
  const hoveredMaterials = useRef(new Map())
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      try {
        // console.log("[ObjectHighlighter] handleClick fired, e.target:", e.target, "gl.domElement:", gl.domElement);
        ...
        // TEMPORARY: Log and do not return
        if (!isPointerLocked.current && e.target !== gl.domElement) {
          // console.log("[ObjectHighlighter] Click NOT on canvas dom element, but not returning for debug");
          // return;
        }
        
        if (hoveredObject && onObjectSelected && !isUIFocused) {
          // console.log(`[ObjectHighlighter] Click on object: name=${hoveredObject.userData?.name || 'unnamed'}, id=${hoveredObject.userData?.id}, uuid=${hoveredObject.uuid}`);
          ...
          if (isShiftKeyPressed) {
            ...
            if (targetObject && (targetObject.userData?.isUserCreated || targetObject.userData?.isSerializedFromCode)) {
              ...
              // console.log(`[ObjectHighlighter] onObjectSelected called with: name=${targetObject.userData?.name || 'unnamed'}, id=${targetObject.userData?.id}, uuid=${targetObject.uuid}`);
            } else {
          ...
          console.log("[ObjectHighlighter] Click: No hovered object or UI is focused");
        }
      } catch (error) {
        console.error("[ObjectHighlighter] Error in handleClick:", error);
      }
    }
    ...
    // console.log("[ObjectHighlighter] window click event listener registered");
    ...
    return () => {
      ...
      // console.log("[ObjectHighlighter] window click event listener removed");
