...
import { CustomTransformControls } from '@/components/three/CustomTransferControl'
...
export function MeshCreator() {
  ...
  useEffect(() => {
    if (!selectedObject) {
      ...
      console.log("[MeshCreator] No selectedObject, clearing selectedInstance");
      return;
    }
    ...
    if (foundObject) {
      ...
      // console.log(`[MeshCreator] Found object in scene: ${objectName}, id=${foundObject.userData?.id}, uuid=${foundObject.uuid}`);
      ...
      if (!foundObject.userData.doubleClickHandler) {
        foundObject.userData.doubleClickHandler = () => {
          ...
          const nextMode = modes[nextIndex]; // No type assertion needed in JS
          setTransformMode(nextMode);
        };
      }
      
      setSelectedInstance(foundObject);
      // console.log("[MeshCreator] setSelectedInstance:", foundObject);
    } else {
      console.warn("[MeshCreator] Object not found in scene, clearing selection");
  ...
  const handleObjectSelected = (object) => {
    // console.log("[MeshCreator] Object selected/deselected:", object);
